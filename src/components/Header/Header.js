...
import { getLocalizedHeaderData } from "../../utils/getLocalizedData";
...
const { social, logo, logo2 } = headerData;

const Header = ({ pageTitle }) => {
  const scrollTop = useScroll(130);
  const { toggleMenu, toggleSearch, isRTL } = useRootContext();
  const { t } = useTranslation();
  
  // Get localized header data
  const localizedHeaderData = getLocalizedHeaderData(t);
  const { icons, navItems } = localizedHeaderData;
              ...
              <ul className="list-unstyled main-header__top-address">
                {icons.map(({ id, icon, content, subHref }) => (
                  <li key={id}>
                    ...
                    <div className="text">
                      <a href={`${subHref}:${content}`} dir="ltr">{content}</a>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="main-header__top-right">
              <div className="main-header__top-right-inner">
                ...
                <div className="main-header__top-right-btn-box">
                  <LanguageSwitcher />
                  <a href="#" className="thm-btn main-header__top-right-btn ms-3">
                    {t('buttons.contactUs')}
                  </a>
                </div>
              </div>
